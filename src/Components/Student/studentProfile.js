...
class studentProfile extends React.Component {
        ...
        usersRef.on("value", snapshot => {
            ...
            for (const [id, fields] of entries) {
              if (id === user.uid) {
                ...
                this.setState({info : fieldArray, userID : user.uid, key : keyHolder[1][0]})
    ...
    updatePhoto () {
        console.log("photo!")
        firebase.database().ref("users/" + this.state.userID + "/" + this.state.key).update({photoID : "Poopy butthole" });
    }

    updateName () {
        console.log("name!")
        firebase.database().ref("users/" + this.state.userID + "/" + this.state.key).update({name : "Poopy butthole" });
    }

    updateSkill () {
        console.log("skills!")
        firebase.database().ref("users/" + this.state.userID + "/" + this.state.key).child('skills').push("butter");
    }

    updateskillLevel () {
        console.log("skillLevel!")
        firebase.database().ref("users/" + this.state.userID + "/" + this.state.key).update({skillLevel : "Poopy buttskill" });
    }

    updatePhone () {
        console.log("Phone!")
        firebase.database().ref("users/" + this.state.userID + "/" + this.state.key).update({phone : "Poopy buttphone" });
    }
    ...
    printEntries = () => {
        ...
        for (const [id, fields] of entries) {
            ...
            return <ul key={fieldArray[0]}> Photo : {fieldArray[1]}, 
              Name: {fieldArray[0]}, 
              Skill Level: {fieldArray[3]}, 
              Phone Number: {fieldArray[2]}
              </ul>
              }
    ...
    return (
      <div>
        <NavbarSt/>
            ...
            <button onClick = {this.updateskillLevel.bind(this)}> Update Skill Level </button>
